...
export function useSalesWizard(
  ...
  // Adding the service (or changing the quote so no rule fires) retires a
  // dismissal: a skip must never outlive the prompt that prompted it, or the
  // quote would report a decline for an add-on it actually carries.
  if (attachDismissal !== null && liveWarning === null) {
  ...
  const dismissAttach = useCallback((reason: string | null) => {
    setAttachDismissal({ reason });
  }, []);
    ...
    async (): Promise<QuoteDetail> => {
      ...
      try {
        const quote = await salesWizardApi.save(workspaceId, {
          ...payload,
          // Only sent once the rep explicitly skips the prompt. The server
          // ignores it when no rule fired, so a stale dismissal can never invent
          // a "they declined" event on a quote that has the attach.
