...
function AgentExperimentSection({ agentId, agentName }: { agentId: string; agentName: string }) {
  ...
  const canManageExperiments = can("workspace:manage");
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Version{" "}
                  {comparison.versions.find((v) => v.version_id === winner_id)?.version_number} —{" "}
                  {(winner_probability * 100).toFixed(1)}% probability best
                </p>
              </div>
            </div>
            <Button
          ...
          <div className="flex items-center justify-between rounded-lg border border-warning/20 bg-warning/10 p-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Version {comparison.versions[comparison.versions.length - 1]?.version_number} is
            ...
            <Button
              ...
              variant="outline"
              onClick={() =>
                setEliminateVersion(comparison.versions[comparison.versions.length - 1].version_id)
              }
              disabled={!canManageExperiments}
                  ...
                  <div className="flex gap-1">
                    {isPaused ? (
                      <Button
                        ...
                        size="icon"
                        className="h-7 w-7"
                        onClick={() => resumeMutation.mutate(version.version_id)}
                        disabled={!canManageExperiments || resumeMutation.isPending}
                      ...
                      <Button
                        ...
                        size="icon"
                        className="h-7 w-7"
                        onClick={() => pauseMutation.mutate(version.version_id)}
                    ...
                    <Button
                      ...
                      onClick={() => setEliminateVersion(version.version_id)}
                      disabled={!canManageExperiments}
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will deactivate all other versions and make this the only active version. The
                A/B test will end.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() =>
                  declareWinnerVersion && activateMutation.mutate(declareWinnerVersion)
                }
                disabled={!canManageExperiments || activateMutation.isPending}
              >
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                disabled={!canManageExperiments || eliminateMutation.isPending}
