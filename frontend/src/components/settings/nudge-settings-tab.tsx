...
export function NudgeSettingsTab() {
  ...
  const toggleChannel = (channel: string, checked: boolean) => {
    ...
    const updated = checked ? [...current, channel] : current.filter((c) => c !== channel);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Get SMS and push reminders about contact birthdays, anniversaries, and follow-ups
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label htmlFor="nudges-enabled">Enable Nudges</Label>
              <p className="text-sm text-muted-foreground">Turn the nudge system on or off</p>
            </div>
            <Switch
              id="nudges-enabled"
              checked={enabled}
      ...
      {enabled && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Control when nudges are generated and sent</CardDescription>
          </CardHeader>
      ...
      {enabled && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Choose which types of nudges you want to receive</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {NUDGE_TYPE_OPTIONS.map((opt) => (
              <div key={opt.type} className="flex items-center justify-between">
                ...
                <Switch
                  id={`nudge-type-${opt.type}`}
                  checked={settings?.nudge_types?.includes(opt.type) ?? false}
                  onCheckedChange={(checked) => toggleNudgeType(opt.type, checked)}
                  disabled={mutation.isPending}
                />
              </div>
            ))}
      ...
      {enabled && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>How you want to receive nudge notifications</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label htmlFor="nudge-channel-sms">SMS</Label>
                <p className="text-sm text-muted-foreground">
                  Requires a phone number in your profile
                </p>
              </div>
              <Switch
                id="nudge-channel-sms"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label htmlFor="nudge-channel-push">Push Notifications</Label>
                <p className="text-sm text-muted-foreground">Real-time alerts in your browser</p>
              </div>
              <Switch
                id="nudge-channel-push"
                checked={settings?.delivery_channels?.includes("push") ?? false}
            ...
            {hasSms && (
              <Alert>
                ...
                <AlertDescription>
                  ...
                  <span className="font-medium underline">Profile</span> settings tab to receive SMS
                  nudges.
                </AlertDescription>
              </Alert>
            )}
      ...
      {enabled && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>No SMS notifications during these hours</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
