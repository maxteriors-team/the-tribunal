 ...
 *
 * Pitch-corrected, so both maps stay true breakdowns of the same footage the
 * top-level `feet` reports — a mismatch would silently skew the
 * measured-feet-weighted markup.
 */
export function permanentRunFeet(
  ...
  const totals: PermanentRunFeet = {
    complexity: { easy: 0, standard: 0, complex: 0 },
