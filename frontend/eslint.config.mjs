...
const eslintConfig = defineConfig([
  ...
  ...nextTs,
  {
    // Scoped to the same extensions `eslint-config-next` registers `jsx-a11y`
    // for. Left unscoped, these rules also apply to files it skips (e.g.
    // `.cjs`), and ESLint aborts the whole run with "could not find plugin
    // jsx-a11y" instead of reporting lint errors.
  ...
  globalIgnores([
    ...
    "test-results/**",
    "next-env.d.ts",
    // Local Netlify build output: ~95M of vendored bundles, never our source.
    ".netlify/**",
