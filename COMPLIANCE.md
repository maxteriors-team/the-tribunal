...
Scope: AI-routed Google Calendar bookings and transactional appointment reminders only. This is engineering guidance, not legal advice.

### Controls implemented and proved

- **CODE/tests:** AI availability and booking routing now requires a linked per-user Google Calendar row; the generated query and AI call-site flag are regression-tested, and no new customer-data surface or provider was added.
- **RUNTIME/integration test with mocked Google:** text bookings commit the CRM appointment before awaiting Google, then return the provider-result Meet URL or the existing truthful failure state; no fabricated URL is emitted.
- **RUNTIME/tests with provider fakes:** video-call SMS/email reminders include the Meet URL, custom templates support `{meeting_url}`, and reminder email HTML safely linkifies it.
...
- `BOOK-001` remains open for the separate fire-and-forget confirmation/rep-email delivery path; synchronous Google event creation only narrows that risk.
- `SMS-002` remains open: adding the meeting URL does not prove the underlying reminder consent basis or carrier registration.