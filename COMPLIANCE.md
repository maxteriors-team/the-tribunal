...
Snapshot: 26 August 2026 · Reviewed by: EZ Coder compliance-guard · **NOT LEGAL ADVICE** · Released at `b1821ef0`

Scope: the transactional customer email sent after each service-invoice payment by Stripe, cash, or check; its persistent delivery outbox; operator-visible delivery/retry controls; and append-only manual settlement provenance. This is a focused engineering review, not a product-wide re-audit.

| ID | Severity | Trigger | Evidence | Obligation | Status | Guard |
|---|---|---|---|---|---|---|
| REC-001 | HIGH | Customers need durable proof of each service-invoice payment | RUNTIME + CODE: each partial or final receipt records invoice number, provided service names and quantities, payment amount, invoice total, total paid, UTC receipt time, and remaining balance or paid-in-full status, with a stable invoice/PDF link | Send accurate, recipient-scoped proof only after a provider-confirmed or authorized offline payment | Fixed | Receipt-renderer tests plus partial→paid→replay PostgreSQL integration tests |
| REC-002 | MEDIUM | A transactional payment email contains customer identity, service names, and payment facts | CODE/tests: recipient and selected service lines come from the paid invoice; customer-authored values use the escaped shared layout; recipient, customer/support, and bounded service-summary snapshots are encrypted; provider IDs, card data, marketing, tracking, and unsubscribe copy are omitted | Minimize payment data and prevent content injection or marketing suppression from contaminating the receipt | Fixed | Escaping, plain-text, transactional-category, encrypted-column, and idempotency-key assertions |
| REC-003 | HIGH | Provider downtime can prevent a receipt after payment is already committed | RUNTIME + CODE: every accepted payment commits an outbox row atomically; a bounded worker claims with row locks, retries with backoff and a stable provider idempotency key, recovers stale claims, and preserves terminal failures | Deliver asynchronously without rolling back or duplicating the successful charge | Fixed | Worker, migration, replay, stale-claim, partial-payment, and retry integration tests |
| REC-004 | MEDIUM | Operators need the recipient and a safe recovery path without seeing provider internals | RUNTIME + CODE: invoice reads expose an allowlisted `pending`/`sent`/`needs_attention`/`skipped` projection; raw `last_error` is not serialized; retry requires `billing:write`, scopes invoice and job by workspace, and only queues/reopens work | Limit payment-delivery metadata to authorized workspace users and keep retries idempotent and asynchronous | Fixed | RBAC, cross-workspace, idempotency, no-inline-send, frontend visibility, and conditional-action tests |
| REC-006 | HIGH | An operator records cash or a check while a customer may still have an open card checkout | RUNTIME + CODE: `billing:write` is required; the workspace-scoped invoice row is locked; an open Stripe checkout is checked and expired before settlement; a concurrent completed card payment wins; each positive amount up to the remaining balance appends a workspace-idempotent ledger row retaining method, encrypted check reference, operator ID, and receipt time | Prevent double collection and leave an auditable, privacy-minimized settlement record before issuing the receipt | Fixed | Cross-workspace, RBAC, partial/final/idempotent replay, encrypted-reference, checkout-expiry, card-wins, receipt-outbox, dialog, and migration tests |
| REC-005 | BLOCKER | A mixed checkout could bypass review or omit generated contracts | RUNTIME: protected-main PR #152 passed required secret, migration, and CodeQL checks plus backend/frontend/Playwright/Vercel gates; production reported exact merged SHA `b1821ef0` and backend/frontend smoke passed | Ship only reviewable commits from current `main`, with deterministic generated contracts and the repository release gates | Fixed | Protected branch, required checks, encrypted production backup, exact-SHA deploy guard, and live smoke |

### Implemented and proved in this pass

- Each accepted partial or final payment snapshots one encrypted, immutable receipt payload—including up to 25 selected service lines and quantities, that payment amount, total paid, and remaining balance—inside the same database transaction as its append-only ledger row.
- A single-process worker sends outside the payment request, retries transient failures, uses provider idempotency, and leaves terminal failures for an operator instead of losing them.
- Billing readers see only the safe delivery projection; only `billing:write` holders can retry or record cash/check settlement, and neither action sends email synchronously.
- Cash/check settlement accepts any positive cent-rounded amount up to the remaining balance and appends method, operator, amount, encrypted optional check number, and receipt time; it expires an open card checkout and lets an already-completed card payment win.
- Release verification passed: backend CI ran 4,813 tests with lint, formatting, and MyPy; frontend CI ran 1,510 tests with lint, type checks, and production build; migration upgrade/check/downgrade/upgrade passed on fresh PostgreSQL; protected-main checks and Playwright passed; production backend smoke passed 6/6 and frontend smoke passed 3/3. Live Resend inbox placement and a full backup restore drill were not exercised.
...
## Focused addendum — Quo single-inbox messaging (2026-08-26)

Snapshot: 26 August 2026 · Reviewed by: EZ Coder compliance-guard · **NOT LEGAL ADVICE** · Uncommitted implementation working tree

Scope: selected-line Quo text synchronization and manual CRM replies for authenticated workspace staff. This addendum records engineering controls, residual risk, and verification boundaries; it is not legal certification and does not establish consent for any real recipient.

| ID | Severity | Trigger | Evidence | Obligation | Status | Guard |
|---|---|---|---|---|---|---|
| QUO-001 | HIGH | Quo text/call resources contain phone numbers, message bodies, call metadata, and provider identifiers for three possible lines | RUNTIME: local signed-webhook, selected-line, cross-tenant, encrypted-credential, and zero-write isolation tests passed; CODE: missing/unselected-line and standalone-contact events are acknowledged without CRM writes and logs are metadata-only | Keep communications tenant- and line-bound, access-controlled, minimized, and absent from operational logs | Engineering control locally verified | Signature, selected-line, cross-tenant, encrypted-credential, and zero-write isolation tests |
| QUO-002 | HIGH | Staff can send SMS and recipients can opt out through any supported provider | CODE: known contact opt-out or matching global opt-out blocks before attempt/network I/O; unknown consent requires inbound SMS on the exact conversation, while new outreach requires recorded opt-in; Quo `STOP` uses the shared opt-out manager | Obtain and retain a valid consent basis, identify the sender where required, and honor opt-outs across every sending path | Engineering gate implemented; lawful consent remains an operator responsibility | Consent, inbound-history, global-opt-out, STOP, and zero-network tests |
| QUO-003 | LAWYER | Call/voicemail activity may expose recording metadata or reflect recorded speech | CODE: selected-line call activity can enter the CRM, but this feature does not establish jurisdiction-specific recording notice, consent, retention, transcript access, or synthetic-voice disclosure rules | Have counsel review calling/recording rules and operational policy for every calling jurisdiction | Open — product/legal decision | Keep Quo outbound calls disabled here; preserve existing recording controls and `AI-001`/`BCALL-003` |
| QUO-004 | MEDIUM | Historical backfill moves existing communications into the CRM | CODE: one workspace, one stored selected line, bounded UTC window, default rollback, explicit `--apply`, aggregate-only output, and idempotent reconciliation constrain writes | Prevent accidental bulk import, line mixing, duplicate activity, and unrecoverable partial writes | Engineering guard implemented; production import not run | Selected-line backfill tests plus encrypted-backup operational runbook |
| QUO-005 | HIGH | Manual CRM replies can become unsolicited or misdirected SMS | CODE: the server derives the provider and sender from the workspace-scoped conversation/integration, rejects sender-line mismatch and media, and disables Quo AI replies, automation, follow-ups, MMS, and outbound calls in this pass | Limit use to consented manual text conversations; keep marketing purpose, quiet-hour, sender-identification, and jurisdictional requirements in the operating policy | Engineering controls implemented; legal/message-purpose review remains outside code | Workspace/sender mismatch, text-only, direct-reply consent, and disabled-feature tests |
| QUO-006 | HIGH | A timeout, disconnect, rate limit, 5xx, or concurrent retry can produce duplicate messages or a false success claim | CODE: one client UUID is claimed before one network attempt; ambiguous outcomes become `unknown`, are not retried, retain the draft, and may reconcile later through a signed webhook; provider IDs map to one canonical row | Do not represent provider acceptance as delivery and do not resend while acceptance is unknown | Engineering control implemented; carrier delivery not verified | Ambiguous transport, stable replay, reverse-order, and concurrent three-writer tests |
| QUO-007 | MEDIUM | Quo processes customer communications as an external provider | DEDUCED: the integration necessarily discloses phone and message data to Quo; this code pass cannot prove contract terms, privacy-notice coverage, retention, deletion propagation, or international-transfer terms | Confirm the vendor contract and public/internal privacy disclosures describe Quo processing, retention, access, deletion, and transfer handling | Open — operational/legal review | Vendor inventory, data-processing agreement review, retention schedule, and deletion playbook |
...
| PPC-A5 | Duplicate-charge/payment handling | pass (code/tests) | Stable Stripe idempotency covers create retries; paid reconciliation survives rollout disablement; transition remains atomic. |
...
| PPC-003 | HIGH | Retried checkout/webhook traffic could charge twice or leave a completed charge unrecorded | CODE/tests: stable provider idempotency, active-session reuse, exact identity verification, and post-disable reconciliation | Fixed for tested paths; atomic paid transition and idempotent notification scope |
