...
| ID | Severity | Trigger | Evidence (RUNTIME / CODE / DEDUCED) | Obligation | Status | Guard |
|---|---|---|---|---|---|---|
| WEB-001 | HIGH | Elementor posts label-keyed Basic data or nested Advanced Data, while Tribunal previously accepted JSON only | RUNTIME: both production attempts returned 422 and created zero leads/messages; the second exposed Elementor's 2.7 KiB nested field/meta shape | Preserve valid quote requests without breaking the form-success event used by ad attribution | Fixed in code; final production proof pending | Basic/Advanced normalization tests, database integration test, and headless HTTP probe |
