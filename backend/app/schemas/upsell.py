...
class UpsellCatalogResponse(BaseModel):
    ...
    # (an office role, or a workspace that configured no cap). Sent so the UI
    # can warn *before* the crew lead builds a proposal they are not allowed to
    # send — the server still enforces it either way.
