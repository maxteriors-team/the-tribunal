...
class Invoice(Base, WorkspaceScoped):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_invoices_manual_payment_amount",
        ),
        UniqueConstraint(
            "workspace_id",
            "manual_payment_idempotency_key",
            name="uq_invoices_manual_payment_idempotency_key",
        ),
    )
    ...
    # The latest provider-accepted invoice destination is receipt routing PII.
    last_emailed_to: Mapped[str | None] = mapped_column(EncryptedString(), nullable=True)
    ...
    # Latest settlement provenance for compatibility; immutable history lives in
    # InvoicePayment. The idempotency key makes a retried operator request a no-op.
    payment_method: Mapped[str | None] = mapped_column(String(20), nullable=True)
    payment_recorded_by_id: Mapped[int | None] = mapped_column(
        Integer,
        ForeignKey("users.id", ondelete="SET NULL"),
        nullable=True,
    )
    manual_payment_amount: Mapped[float | None] = mapped_column(Numeric(12, 2), nullable=True)
    manual_payment_reference: Mapped[str | None] = mapped_column(EncryptedString(), nullable=True)
    manual_payment_idempotency_key: Mapped[uuid.UUID | None] = mapped_column(
    ...
    payment_recorded_by: Mapped["User | None"] = relationship(
        "User", foreign_keys=[payment_recorded_by_id]
    )
