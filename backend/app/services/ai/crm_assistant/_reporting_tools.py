...
_TEAMS = ("reception", "technicians", "office_reps")
...
# ``/scorecard/office-reps`` refuses a window wider than a year; the same bound is
...
class ReportingAssistantTools:
    ...
    async def get_scorecard(self, args: ToolArguments) -> dict[str, object]:
        ...
        rows = (
            await service.get_technician_activity(workspace, start_date, end_date)
            if team == "technicians"
            else await service.get_office_rep_activity(workspace, start_date, end_date)
        )
        return {
            "success": True,
            "team": team,
