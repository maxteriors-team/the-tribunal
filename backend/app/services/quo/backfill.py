...
class QuoHistoricalBackfill:
    ...
    def __init__(
        ...
        *,
        ...
        self._contact_ids_by_phone: dict[str, list[str]] = {}
    ...
    async def run(self) -> QuoBackfillCounts:
        """Run the bounded import and rollback unless apply was explicit."""
        try:
            await self._import_contacts()
            phone_numbers = await self._phone_numbers()
            if phone_numbers:
    ...
    async def _import_contacts(self) -> None:
        try:
            async for resource in self.client.iter_contacts():
                counts = self.counts.contacts
                counts.seen += 1
                try:
                    event, phones = _contact_event(resource, self.organization_id)
                    if not _resource_touches_window(resource, self.since, self.until):
                        counts.skipped += 1
                        continue
                    resource_id = _resource_id(resource, "contact")
                    if resource_id in self._seen_contact_ids:
                        counts.skipped += 1
                        continue
                    self._seen_contact_ids.add(resource_id)
                    counts.eligible += 1
                    if await self._sync(event, counts):
                        for phone in phones:
                            ids = self._contact_ids_by_phone.setdefault(phone, [])
                            if resource_id not in ids and len(ids) < 10:
                                ids.append(resource_id)
                except QuoBackfillDataError:
                    counts.errors += 1
        except QuoApiError as exc:
            self._record_api_error(exc, self.counts.contacts)

    async def _phone_numbers(self) -> dict[str, str]:
        try:
        ...
        for resource in resources:
            try:
                phone_number_id = _required_string(resource.get("id"), "phone number ID")
                phone_number = _phone(resource.get("number"), "workspace phone number")
                existing = phone_numbers.get(phone_number_id)
                if existing is not None and existing != phone_number:
                    raise QuoTenantMismatchError("Quo returned conflicting workspace phone numbers")
                phone_numbers[phone_number_id] = phone_number
            except QuoBackfillDataError:
                self.counts.conversations.errors += 1
    ...
    async def _import_messages(self, phone_number_id: str, participant: str) -> None:
        try:
            ...
            async for resource in messages:
                await self._import_message(resource, phone_number_id, participant)
        except QuoApiError as exc:
        ...
        try:
            ...
            await self._sync(
                _message_event(
                    ...
                    contact_ids=self._contact_ids_by_phone.get(participant, []),
                ),
                counts,
            )
        except QuoBackfillDataError:
...
def _contact_event(
    resource: dict[str, Any], organization_id: str
) -> tuple[QuoWebhookEvent, list[str]]:
    resource_id = _resource_id(resource, "contact")
    default_fields = resource.get("defaultFields")
    if not isinstance(default_fields, dict):
        raise QuoBackfillDataError("Quo contact fields are invalid")
    phones = _field_values(default_fields.get("phoneNumbers"), normalize_phone=True)
    emails = _field_values(default_fields.get("emails"), normalize_phone=False)
    event_resource = {
        "id": resource_id,
        "firstName": default_fields.get("firstName"),
        "lastName": default_fields.get("lastName"),
        "company": default_fields.get("company"),
        "role": default_fields.get("role"),
        "phoneNumbers": phones,
        "emails": emails,
    }
    occurred_at = _latest_resource_timestamp(resource)
    return _event(
...
def _resource_touches_window(resource: dict[str, Any], since: datetime, until: datetime) -> bool:
    timestamps = [
        _provider_timestamp(resource.get(key), f"contact {key}")
        for key in ("createdAt", "updatedAt")
        if resource.get(key) is not None
    ]
    if not timestamps:
        raise QuoBackfillDataError("Quo contact timestamps are missing")
    return any(since <= timestamp < until for timestamp in timestamps)
...
def _latest_resource_timestamp(resource: dict[str, Any]) -> datetime:
    values = [
        _provider_timestamp(resource.get(key), f"contact {key}")
        for key in ("createdAt", "updatedAt")
        if resource.get(key) is not None
...
def _field_values(value: object, *, normalize_phone: bool) -> list[str]:
    if not isinstance(value, list):
        raise QuoBackfillDataError("Quo contact phone fields are invalid")
    result: list[str] = []
    for item in value:
        raw_value = item.get("value") if isinstance(item, dict) else None
        if normalize_phone:
            normalized = normalize_phone_safe(raw_value) if isinstance(raw_value, str) else None
            if normalized and normalized not in result:
                result.append(normalized)
        elif isinstance(raw_value, str):
            normalized = raw_value.strip()
            if normalized and len(normalized) <= 320 and normalized not in result:
                result.append(normalized)
    return result
