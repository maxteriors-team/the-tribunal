...
_PUBLIC_CAPABILITY_PATTERN = re.compile(r"[A-Za-z0-9_-]{1,128}\Z")
...
def _raise_intake_not_found() -> NoReturn:
    raise HTTPException(
        ...
        detail="Referral partner intake not found",
...
def _public_bearer_token(
    ...
    if (
        ...
        or _PUBLIC_CAPABILITY_PATTERN.fullmatch(credentials.credentials) is None
    ):
...
async def upload_public_referral_partner_logo(
    token: PublicIntakeToken, request: Request, db: TransactionalDB, response: Response
) -> ReferralPartnerLogoResponse:
