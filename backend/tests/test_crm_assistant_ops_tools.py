...
async def test_scorecard_window_is_bounded(db: MagicMock, workspace_id: uuid.UUID) -> None:
    ...
    result = await _executor(db, workspace_id).execute(
        "get_scorecard", {"team": "technicians", "date_from": "2020-01-01", "date_to": "2026-01-01"}
    )
...
async def test_every_offered_scorecard_team_is_actually_implemented(
    db: MagicMock, workspace_id: uuid.UUID
) -> None:
    """Each team in the tool schema must reach a real ScorecardService method.

    The enum advertised to the model and the branches this tool can service are
    two lists that drift silently: offering a team whose service method does not
    exist produces a tool call that always fails. This pins them together.
    """
    from app.services.ai.crm_assistant._reporting_tools import _TEAMS
    from app.services.dashboard.scorecard_service import ScorecardService

    schema = next(
        tool["function"] for tool in get_crm_tools() if tool["function"]["name"] == "get_scorecard"
    )
    advertised = set(schema["parameters"]["properties"]["team"]["enum"])
    assert advertised == set(_TEAMS)

    required = {"reception": "get_scorecard", "technicians": "get_technician_activity"}
    for team in advertised:
        assert hasattr(ScorecardService, required[team]), team
