...
class TestAuth:
    ...
    async def test_delete_requires_auth(self, noauth_client: AsyncClient) -> None:
        # The request stays outside the assert: `python -O` strips asserts, and an
        # unauthenticated-DELETE test that never issues the DELETE passes vacuously.
        response = await noauth_client.delete(_base(f"/{PARTNER_ID}"))
        assert response.status_code == 401
