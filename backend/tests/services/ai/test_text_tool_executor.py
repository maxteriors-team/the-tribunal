...
def test_video_booking_success_returns_provider_meet_link() -> None:
    executor = TextToolExecutor.__new__(TextToolExecutor)
    executor._appointment_datetime = datetime.fromisoformat("2026-08-12T13:30:00-04:00")
    ...
    assert "Google Meet link is https://meet.google.com/abc-defg-hij" in result["message"]
...
async def test_live_sms_booking_suppresses_generic_confirmation() -> None:
    ...
    assert finalize.await_args.kwargs["sync_calendar_before_return"] is True
