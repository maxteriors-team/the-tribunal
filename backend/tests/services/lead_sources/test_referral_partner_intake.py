...
@pytest.mark.asyncio
async def test_issue_reuses_then_rotates_and_revokes_high_entropy_token() -> None:
    db = _db()
    service = ReferralPartnerIntakeService(db)
    partner = _partner()
    service._scoped_partner = AsyncMock(return_value=partner)  # type: ignore[method-assign]
...
@pytest.mark.asyncio
async def test_repeat_submit_updates_same_partner_without_creating_duplicate() -> None:
    db = _db()
    service = ReferralPartnerIntakeService(db)
    partner = _partner()
    ...
    service._resolve_token = AsyncMock(return_value=(link, partner))  # type: ignore[method-assign]
    service.public_prefill = AsyncMock(return_value=MagicMock())  # type: ignore[method-assign]
...
@pytest.mark.asyncio
async def test_logo_auth_precedes_size_and_magic_validation() -> None:
    db = _db()
    service = ReferralPartnerIntakeService(db)
    service._resolve_token = AsyncMock(  # type: ignore[method-assign]
        side_effect=ReferralPartnerIntakeNotFoundError
    )
    with pytest.raises(ReferralPartnerIntakeNotFoundError):
    ...
    service._resolve_token = AsyncMock(  # type: ignore[method-assign]
        return_value=(MagicMock(), _partner())
    )
...
async def test_logo_route_authenticates_before_bounded_multipart_parse(
    ...
    await intake_routes.upload_public_referral_partner_logo(
        "secret-capability", request, MagicMock()
    )
