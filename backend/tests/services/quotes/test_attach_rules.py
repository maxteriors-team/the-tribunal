...
def test_corrupt_blob_falls_back_to_defaults() -> None:
    ...
    workspace = FakeWorkspace(
        {SETTINGS_KEY: {"rules": "not-a-list", "enabled": {"nope": True}}}
    )
...
def test_missing_attach_returns_a_warning() -> None:
    ...
    warning = evaluate_attach_rules(
        _config(), primary_service="roof", present_categories=["roof"]
    )
...
def test_unmatched_primary_service_is_silent() -> None:
    assert (
        evaluate_attach_rules(
            _config(), primary_service="windows", present_categories=["windows"]
        )
        is None
    )
...
def test_blocking_mode_is_reported_on_the_warning() -> None:
    config = AttachRulesSettings(
        rules=[
            AttachRule(
                primary_category="siding", suggested_categories=["trim"], mode="blocking"
            )
        ]
    )

    warning = evaluate_attach_rules(
        config, primary_service="siding", present_categories=["siding"]
    )
...
def test_warning_carries_the_dismissal_vocabulary() -> None:
    ...
    warning = evaluate_attach_rules(
        config, primary_service="roof", present_categories=["roof"]
    )
