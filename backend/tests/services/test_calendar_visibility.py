...
@pytest.mark.parametrize("action", ["update", "delete", "remind"])
async def test_appointment_mutations_refuse_unbooked_row(action: str) -> None:
    """A field caller cannot mutate or text for another worker's appointment."""
    async with AsyncSessionLocal() as db:
        ws = await _workspace(db)
        contact = await _contact(db, ws.id)
        user = await _user(db)
        ...
        with pytest.raises(HTTPException) as excinfo:
            if action == "update":
                ...
                await service.delete_appointment(ws.id, theirs.id, visible_to_user_id=user.id)
            else:
                await service.send_reminder(ws.id, theirs.id, ws, visible_to_user_id=user.id)
