...
| SEC-001 | **PARTIALLY RESOLVED 2026-09-06** | **CODE:** the legacy custom telemetry client, server hook, endpoint configuration, and dependency were removed. Sentry still initializes globally without a source-enforced notice, processor record, buyer-owned account, or consent/legitimate-interest decision. | New builds no longer send browser errors to the custom external endpoint; Sentry governance and payload-minimization risks remain. | Document Sentry ownership and processing, minimize payloads, and gate non-essential telemetry as counsel directs. | The custom phone-home path is removed; the remaining Sentry governance work still matters before handoff. |
...
| A11Y-001 | **CLOSED** | **CODE:** the legacy Sales Wizard controls and stylesheet were removed; `frontend/src/app/sales-wizard/page.tsx` now redirects saved bookmarks into the consolidated Quotes & Estimates designer. | The source-visible missing-focus defect no longer ships. | Keep keyboard and serious/critical axe coverage on the consolidated designer; complete the separate manual assistive-technology review before making conformance claims. | Reintroducing custom controls without visible focus would reopen this finding. |
...
| A5 | Focus visibility | n-a | The affected legacy Sales Wizard controls were removed; `/sales-wizard` redirects to the consolidated designer. Manual focus verification remains outside this historical audit. |
