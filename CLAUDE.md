...
- `GET /version` reports the deployed commit. Railway injects `RAILWAY_GIT_COMMIT_SHA` only for **git-triggered** builds, so `make deploy.backend` sets the **`BUILD_COMMIT_SHA` service variable** (`--skip-deploys`, so it never ships old code under a new SHA) immediately before `railway up`; the app reads it at runtime, independent of what the upload contains. `/version` then reports `"source":"build_env"`. The older `backend/app/build_info.json` stamp is still written and still read as a fallback, but it does **not** survive a manual deploy on its own: `railway up` builds its upload from git's view of the tree and the stamp is deliberately untracked, so it never reaches the builder — which is why every manual deploy reported `"unknown"` until 2026-08-28. Keep the stamp **out of git** anyway (a committed stamp makes `/version` report a stale SHA — worse than `"unknown"`; a pre-commit hook blocks it).
...
- Production URLs: backend `https://the-tribunal-api-production.up.railway.app` (Railway project `the-tribunal`, service `the-tribunal-api`), frontend `https://app.maxteriorslighting.com` (Vercel project `the-tribunal`, team `maxteriors`). `https://the-tribunal-two.vercel.app` is a **still-live alias** onto the same deployment (verified 2026-08-28: byte-identical response, and backend CORS allows both origins), so links already sent to customers keep working — use the `app.maxteriorslighting.com` domain for anything new, and treat both as production when changing `CORS_ORIGINS`, `FRONTEND_URL`, or provider webhook URLs.
...
5. **Deploy the backend from the merged `main`, not from your branch.** `railway up` uploads a **folder from disk**, not a git ref, so whatever is on disk becomes production: deploying a checkout that is missing merged commits silently **reverts them in prod**. Always `git fetch && git reset --hard origin/main` (or `git pull --ff-only`) *first*, then `make deploy.backend`. The script now refuses to deploy when `origin/main` has backend commits the checkout lacks — override only for a deliberate rollback with `DEPLOY_ALLOW_BEHIND=1`. Note a rebase-merge **rewrites SHAs**, so the SHA you tested locally is not the SHA on `main`; deploy the rewritten one so `/version` matches a commit that actually exists.
   - **The upload runs from the repo root, not from `backend/`.** The Railway service's **Root Directory is already `backend`**, and it applies that to whatever tree is uploaded — so uploading from inside `backend/` makes the builder look for `backend/backend/` and the build dies with `Failed to read app source directory / nixpacks exited with an error`. That failure is safe but misleading: the build never produces an image, so prod keeps serving the old release and `/version` still reports the **old** SHA, which reads exactly like a deploy that did nothing. `make deploy.backend` handles this; don't hand-run `railway up` from `backend/`.
   - Because the upload is rooted at the repo, the root **`.railwayignore`** is what filters it (`backend/.railwayignore` no longer governs). It excludes `frontend/` (~858M of `node_modules`) and `backend/.venv` (~489M) — without it a deploy from a real working checkout tries to tar ~5G. A fresh clone has neither, so this only bites on a developer machine, which is where deploys are run from.
